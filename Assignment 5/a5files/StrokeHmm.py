...
class HMM:
    ...
    def label( self, data ):
        ...
        tmp = {}
        for s in self.states:
            tmp[s] = 0 # initialize tmp to 0 with the size of states
        for i in range(len(data)):
            tmpSProb.append(copy.deepcopy(tmp)) # initialize temporal probability to 0
            tmpState.append("test") # initialize temporal states list to "test"
        for i in range(len(data)):
            if i == 0: # partial probability at first time
                maxStateProb = 0.0
                for state in self.states:
                    # at first time, partial probability = prior probability of state * probability of evidence
                    tmpSProb[0][state] = self.priors[state] * self.getEmissionProb(state, data[0])
                    # update the max state probability and states list at first time
                    if (tmpSProb[0][state] > maxStateProb):
                        maxStateProb = tmpSProb[0][state]
                        tmpState[0] = state
            else:
                maxStateProb = 0.0
                for state in self.states:
                    ...
                    for pre_state in self.states:
                        # temp probability = partial probability of previous state * transition * evidence
                        temp = tmpSProb[i-1][pre_state]*self.transitions[state][pre_state]*self.getEmissionProb(state, data[i])
                        # update the max probability in one state
                        if temp > maxProb:
                            maxProb = temp
                            tmpSProb[i][state] = maxProb
                        # update the max state probability and states list at time i
                    if tmpSProb[i][state] > maxStateProb:
                        maxStateProb = tmpSProb[i][state]
                        tmpState[i] = state
        return tmpState

# Example - using the example from class
...
class StrokeLabeler:
    def __init__(self):
        ...
        # another feature we choose is box area
        self.featureNames = ['length', 'curvature', 'boxArea', 'HWRatio']
        self.contOrDisc = {'length': DISCRETE, 'curvature': CONTINUOUS, 'boxArea': CONTINUOUS, 'HWRatio': DISCRETE}
        self.numFVals = {'length': 2, 'HWRatio': 2}
    ...
    def trainHMMDir( self, trainingDir ):
        ...
        for f in tFiles:
            s, trueLables = self.loadLabeledFile( f )
            stroke = self.loadStrokeFile( f )
            labels = self.labelStrokes(stroke)
            cfs = self.confusion(trueLables, labels)
            count['drawing']['drawing'] += cfs['drawing']['drawing']
            count['drawing']['text'] += cfs['drawing']['text']
            count['text']['drawing'] += cfs['text']['drawing']
            count['text']['text'] += cfs['text']['text']
            # res.append(self.confusion(trueLables, labels))
            # print {'drawing':{'drawing': drawCount[1], 'text': drawCount[0]}, 'text':{'drawing':textCount[0], 'text': textCount[1]}}
        print count
    ...
    def featureTest( self, strokeFile ):
        ...
        for i in range(len(strokes)):
            ...
            print "Height/width ratio is", strokes[i].HWRatio()
    ...
    def labelFile( self, strokeFile, outFile ):
        ...
        # s, trueLables =  self.loadLabeledFile(strokeFile)
        labels = self.labelStrokes( strokes )
        # print self.confusion(trueLables, labels)
        print "Labeling done, saving file as", outFile
        self.saveFile( strokes, labels, strokeFile, outFile )

    def confusion(self, trueLables, classfication):
        '''takes a list of truth labels and a list of labels output by the classifier for the same set of strokes (in order)'''
        textCount = [0,0]
        drawCount = [0,0]
        for i in range(len(trueLables)):
            if trueLables[i] == "text":
                if classfication[i] == "text":
                    textCount[1] += 1   # correct
                else:
                    textCount[0] += 1   # wrong
            else:
                if classfication[i] == "drawing":
                    drawCount[1] += 1   # correct
                else:
                    drawCount[0] += 1   # wrong
        print {'drawing':{'drawing': drawCount[1], 'text': drawCount[0]}, 'text':{'drawing':textCount[0], 'text': textCount[1]}}
        # print textCount
        # print drawCount
        return {'drawing':{'drawing': drawCount[1], 'text': drawCount[0]}, 'text':{'drawing':textCount[0], 'text': textCount[1]}}
...
class Stroke:
    ...
    def boxArea(self):
        ...
        min_X = sys.maxint
        min_Y = sys.maxint
        # finding the max x, max y, min x and min y.
        for point in self.points[0:]:
            max_X = max(max_X, point[0])
            max_Y = max(max_Y, point[1])
            min_X = min(min_X, point[0])
            min_Y = min(min_Y, point[1])
        # area = delta(y) * delta(x) = (max y - min y)(max x - min x)
        area = (max_Y - min_Y) * (max_X - min_X)
        return area

    def HWRatio(self):
        '''Return the box height/width ration'''
        xCoordinates =  []
        yCoordinates = []
